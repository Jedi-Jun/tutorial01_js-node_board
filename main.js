...
const server = http.createServer(function(request, response) {
    ...
    if(pathName === '/') {
        ...
        fs.readdir('./data', 'utf8', function(err, fileList) {
            ...
            fs.readFile(`./data/${title}`, function(err, description) {
                if(description === undefined) {
                    ...
                    let template = templateHTML(title, list, control, `<h2>${title}</h2><p>${description}</p>`);
                    response.writeHead(200);
                    response.end(template);
                } else {
                    let control = templateControl();
                    let template = templateHTML(title, list, control, `<h2>${title}</h2><p>${description}</p>`);
                    response.writeHead(200);
                    response.end(template);
                }
        ...
        fs.readdir('./data', 'utf8', function(err, fileList) {
            ...
            let list = templateList(fileList);
            let title = 'CREATE';
            let control = templateControl('', 'disabled', 'disabled');
            let template = templateHTML(title, list, control, `
            <p>
